...
public class FiscalDocument {
    ...
    public FiscalDocument(NfeDTO nfeDTO) {
        docNumber = nfeDTO.getDocNumber();
        value =  nfeDTO.getValue();
        emissionDate = convertDate(nfeDTO.getEmissionDate());
        docValueType = DocValueType.EXPENSE;
        docType = DocType.NFE;
    }

    private LocalDate convertDate(String dhEmi){
        final int DATE_SIZE = 9;
        String date = dhEmi.substring(1,DATE_SIZE);
    ...
    public boolean isExpense() {
        return docValueType == DocValueType.EXPENSE;
    }

    public boolean isIncome() {
        return docValueType == DocValueType.INCOME;
    }
